...
class MainActivity : Activity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (PROCESS_SESSION_INITIALIZED.compareAndSet(false, true)) {
            resetRuntimeServicesToOff()
        }
        selectedOutputTreeUri = preferences.getString(KEY_OUTPUT_TREE_URI, null)?.let(Uri::parse)
        selectedRecordingPath = preferences.getString(
            KEY_RECORDING_DIRECTORY,
            AppConfig.defaultRecordingDirectory.absolutePath
        ) ?: AppConfig.defaultRecordingDirectory.absolutePath
        selectedOutputPath = preferences.getString(
    ...
    private fun resetRuntimeServicesToOff() {
        preferences.edit()
    ...
    private fun buildContentView(): View {
        ...
        startButton = Button(this).apply {
            ...
            background = violetButtonBackground(density)
        ...
        val wifiSection = LinearLayout(this).apply {
            ...
            addView(wifiButton, matchWrap())
            addView(wifiUrlView, matchWrap())
        }

        advancedButton = compactButton("ADVANCED ▼") { toggleAdvanced() }
        advancedSection = LinearLayout(this).apply {
            orientation = LinearLayout.VERTICAL
            visibility = View.GONE
            addView(compactButton("RESET RECORDING FOLDER") {
            ...
            addView(compactButton("RESET OUTPUT FOLDER") {
                selectedOutputTreeUri = null
                selectedOutputPath = AppConfig.defaultOutputDirectory.absolutePath
                outputPathView.text = compactPath(selectedOutputPath)
            })
            addView(TextView(this@MainActivity).apply {
                text = "Valid GPS always produces a GPX. Recordings with gaps over 5 seconds are logged in FAILED.TXT and moved with their GPX into the output folder's Failed subfolder."
                textSize = 11f
        ...
        val content = LinearLayout(this).apply {
            ...
            addView(advancedButton, matchWrap())
    ...
    private fun folderRow(
        ...
        return LinearLayout(this).apply {
            ...
            addView(compactButton(buttonText, lightBlue = true, action = action), matchWrap())
        }
    }
    ...
    private fun compactButton(
        ...
        text = label
        textSize = 12f
        val requestedHeight = heightDp?.let { (it * density).toInt() } ?: 0
        minHeight = requestedHeight
        minimumHeight = requestedHeight
        setPadding(6, 2, 6, 2)
        when {
            violet -> {
                setTypeface(typeface, Typeface.BOLD)
            ...
            lightBlue -> {
                setBackgroundColor(Color.rgb(129, 212, 250))
    ...
    private fun violetButtonBackground(density: Float): GradientDrawable = GradientDrawable().apply {
        shape = GradientDrawable.RECTANGLE
        cornerRadius = 12f * density
        setColor(Color.rgb(126, 87, 194))
    }
    ...
    private fun updateStatus(status: String) {
        val serviceActive = status != RecordingMonitorService.STATUS_IDLE
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        preferences.edit()
            ...
            .apply()
        Toast.makeText(this, "SD-card output permission granted", Toast.LENGTH_LONG).show()
    }
    ...
    private fun toggleMonitoring() {
        val lastStatus = preferences.getString(
            RecordingMonitorService.KEY_LAST_STATUS,
            RecordingMonitorService.STATUS_IDLE
        ) ?: RecordingMonitorService.STATUS_IDLE
        if (lastStatus != RecordingMonitorService.STATUS_IDLE) {
            stopMonitoring()
        } else {
            saveDirectoriesAndStartMonitoring()
        }
    }
    ...
    private fun saveDirectoriesAndStartMonitoring() {
        ...
        if (outputUri == null) {
            ...
            if (input.canonicalPath == output.canonicalPath) {
                Toast.makeText(this, "Input and output folders must differ", Toast.LENGTH_LONG).show()
                return
            }
        } else {
            val resolved = DocumentTreePathResolver.resolve(this, outputUri)
            if (resolved != null && input.canonicalPath == resolved.canonicalPath) {
                Toast.makeText(this, "Input and output folders must differ", Toast.LENGTH_LONG).show()
                return
            }
        ...
        updateStatus(RecordingMonitorService.STATUS_MONITORING)
    ...
    private fun verifyLocalFolderAccess(directory: File, label: String): String? {
        ...
        return try {
            ...
            if (!probe.isFile || probe.length() != 3L) {
                ...
                if (bytes.size != 3) "$label folder failed the read-back test: ${directory.absolutePath}" else null
            }
    ...
    private fun verifyDocumentTreeAccess(treeUri: Uri): String? {
        ...
        return try {
            ...
            if (bytes.size != 3) "Output folder failed the read-back test." else null
        } catch (error: Throwable) {
    ...
    private fun updateWifiServerUi(active: Boolean) {
        ...
        wifiUrlView.text = if (active) {
            ...
            if (address == null) "Connect camera to Wi-Fi" else "http://$address:${WifiFileServerService.PORT}"
        } else ""
    ...
    companion object {
        ...
        private val PROCESS_SESSION_INITIALIZED = AtomicBoolean(false)
    }
}
