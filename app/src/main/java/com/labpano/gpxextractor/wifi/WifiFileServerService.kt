...
class WifiFileServerService : Service() {
        ...
        acceptThread = Thread({
            try {
                ServerSocket().use { server ->
                    ...
                    server.bind(InetSocketAddress(InetAddress.getByName("0.0.0.0"), PORT), BACKLOG)
                    serverSocket = server
                    setEnabled(true)
                    publishStatus(true, null)
                    AppLog.info("Wi-Fi file server listening on port $PORT")
                    while (running.get()) {
                        val socket = try {
                            server.accept()
                ...
                setEnabled(false)
                publishStatus(false, error.message ?: error.javaClass.simpleName)
                stopSelf()
            }
    ...
    private fun route(
        ...
        if (path == "/api/v1/dashboard") {
            if (method != "GET" && method != "HEAD") {
                ...
                sendJson(output, DashboardApi.build(this), headOnly)
            }
            return
        }
    ...
    private fun buildPendingGpxQueue(query: String): String {
        ...
        try {
            store.prune()
            if (store.pendingGpxCount() == 0L) importLegacyPendingQueue(store)
    ...
    private fun importLegacyPendingQueue(store: ProcessedRecordingStore) {
        ...
        val monitoringPath = preferences.getString(MainActivity.KEY_RECORDING_DIRECTORY, null)
        ...
        listOf("GOOD" to "GOOD.TXT", "FAILED" to "FAILED.TXT").forEach reports@ { (status, reportName) ->
            val report = File(monitoringPath, reportName)
...
a{color:#075fa8;text-decoration:none;word-break:break-word}small{display:block;color:#687078;margin-top:4px}.entry{display:flex;gap:12px;align-items:center;justify-content:space-between}.download{flex:none;background:#6f42c1;color:white;padding:7px 11px;border-radius:6px}.notice{background:#eaf3ff;border-left:4px solid #075fa8;padding:12px;border-radius:6px}.muted{color:#687078}
    ...
    private fun buildNotification(): Notification {
        ...
        return builder.setSmallIcon(R.drawable.ic_app)
            ...
            .setContentText("Read-only browser available on port $PORT")
