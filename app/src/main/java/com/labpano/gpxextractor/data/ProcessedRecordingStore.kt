...
class ProcessedRecordingStore(context: Context) : SQLiteOpenHelper(
    ...
    private fun createTransferJournalTable(db: SQLiteDatabase) {
        ...
        db.execSQL(
            "CREATE INDEX IF NOT EXISTS idx_journal_source_state ON transfer_journal(source_path, source_size, source_modified_at, state)"
        )
    }
    ...
    fun hasPendingTransaction(path: String, fileSize: Long, modifiedAt: Long): Boolean {
        readableDatabase.query(
            "transfer_journal",
            arrayOf("transaction_id"),
            "source_path = ? AND source_size = ? AND source_modified_at = ? AND state != ?",
            arrayOf(path, fileSize.toString(), modifiedAt.toString(), STATE_COMMITTED),
            null,
            null,
            null,
            "1"
    ...
    fun prune(now: Long = System.currentTimeMillis()) {
        ...
        val finalCutoff = now - AppConfig.PENDING_QUEUE_RETENTION_MS
        val staleFinalPaths = mutableListOf<String>()
        database.query(
            "processed_recordings",
            arrayOf("path"),
            "is_final = 1 AND processed_at < ?",
            arrayOf(finalCutoff.toString()),
            null,
            null,
            null
        ).use { cursor ->
            while (cursor.moveToNext()) {
                ...
                if (!File(path).exists()) staleFinalPaths += path
            }
        }
        staleFinalPaths.forEach { path ->
            database.delete(
                "processed_recordings",
                "path = ? AND is_final = 1 AND processed_at < ?",
                arrayOf(path, finalCutoff.toString())
            )
        }
        database.delete(
            "transfer_journal",
