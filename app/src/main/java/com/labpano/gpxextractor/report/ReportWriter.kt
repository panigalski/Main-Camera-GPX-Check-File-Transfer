...
class ReportWriter(private val reportDirectory: File) {
    ...
    private fun trimIfNeeded(report: File) {
        if (report.length() <= AppConfig.MAX_ROLLING_REPORT_BYTES) return
        val lastLines = ArrayDeque<String>(AppConfig.MAX_ROLLING_REPORT_LINES)
        report.useLines(Charsets.UTF_8) { lines ->
            lines.forEach { line ->
                if (lastLines.size == AppConfig.MAX_ROLLING_REPORT_LINES) lastLines.removeFirst()
                lastLines.addLast(line)
            }
        }
        val temporary = File(report.parentFile, ".${report.name}.trim-${System.nanoTime()}.tmp")
        ...
        val backup = File(report.parentFile, ".${report.name}.backup")
        if (backup.exists()) backup.delete()
        if (!report.renameTo(backup)) {
            temporary.delete()
            return
        }
